...
use super::{only_sync, only_async};
...
pub trait BusOperation {
    ...
    fn write_byte_read_bytes(&mut self, wbuf: &[u8; 1], rbuf: &mut [u8]) -> impl Future<Output = Result<(), Self::Error>>;

    #[inline]
    fn read_from_register(&mut self, reg: u8, buf: &mut [u8]) -> impl Future<Output = Result<(), Self::Error>> {
        async move {
            self.write_byte_read_bytes(&[reg], buf).await
        }
    }

    #[inline]
    fn write_to_register(&mut self, reg: u8, buf: &[u8]) -> impl Future<Output = Result<(), Self::Error>> {
        async move {
            let mut tmp: [u8; CHUNK_SIZE + 1] = [0; CHUNK_SIZE + 1];
            let mut reg = reg;
            for chunk in buf.chunks(CHUNK_SIZE) {
...
pub trait RegisterOperation<D, E> {
    ...
    fn read (sensor: &mut D) -> Result<Self, E> where Self: Sized;
...
pub trait RegisterOperation<D, E> {
    ...
    fn read (sensor: &mut D) -> impl Future<Output = Result<Self, E>> where Self: Sized;
